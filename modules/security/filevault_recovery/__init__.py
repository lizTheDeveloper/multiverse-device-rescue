...
class Module(ModuleBase):
    ...
    def check(self, profile: SystemProfile) -> CheckResult:
        ...
        # If FileVault is off, just report INFO status (encryption_check handles the CRITICAL)
        if not is_fv_enabled:
            findings.append(
                Finding(
                    title="FileVault is disabled",
                    description=(
                        "FileVault encryption is not enabled. Recovery key checks "
                        "are not applicable until FileVault is enabled."
                    ),
        ...
        findings.append(
            Finding(
                title="FileVault recovery status",
                description=(
                ...
                actions.append(
                    Action(
                        ...
                        description=(
                            ...
                            "On family devices, consider using an institutional recovery key "
                            "managed by your organization instead."
                        ),
                        risk_level=RiskLevel.SAFE,
                        success=True,
                    )
