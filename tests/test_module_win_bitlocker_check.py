...
ADDITIONAL_DRIVE_NOT_ENCRYPTED = """
C:
    Device Name: C:
    Mount Point: C:\\
    Status: FullyEncrypted
...
def _get_module():
    ...
    return next(m for m in modules if m.name == "win_bitlocker_check")
...
def test_win_bitlocker_check_discovered():
    mod = _get_module()
    assert mod.name == "win_bitlocker_check"
    assert mod.category == "security"
    assert Platform.WIN32 in mod.platforms
    assert mod.risk_level == RiskLevel.SAFE


def test_win_bitlocker_check_healthy_system():
    mod = _get_module()
...
def test_win_bitlocker_check_os_drive_not_encrypted():
    mod = _get_module()
...
def test_win_bitlocker_check_encryption_suspended():
    mod = _get_module()
...
def test_win_bitlocker_check_no_recovery_key():
    mod = _get_module()
...
def test_win_bitlocker_check_mixed_volumes():
    mod = _get_module()
...
def test_win_bitlocker_check_additional_drive_not_encrypted():
    mod = _get_module()
    with patch("subprocess.run", side_effect=_fake_run(ADDITIONAL_DRIVE_NOT_ENCRYPTED)):
        result = mod.check(_make_profile())
    assert result.has_issues
    # Should have a WARNING for unencrypted D: drive
    warnings = [f for f in result.findings if f.severity == Severity.WARNING]
    assert len(warnings) > 0
    assert any("D:" in w.title and "not encrypted" in w.title.lower() for w in warnings)


def test_win_bitlocker_check_manage_bde_error():
    mod = _get_module()
...
def test_win_bitlocker_check_fix_is_informational():
    mod = _get_module()
    with patch("subprocess.run", side_effect=_fake_run(OS_DRIVE_NOT_ENCRYPTED)):
...
def test_win_bitlocker_check_fix_suspended_encryption():
    mod = _get_module()
    with patch("subprocess.run", side_effect=_fake_run(ENCRYPTION_SUSPENDED)):
        check = mod.check(_make_profile())
        fix = mod.fix(check, Mode.MANUAL)
...
def test_win_bitlocker_check_fix_no_recovery_key():
    mod = _get_module()
    with patch("subprocess.run", side_effect=_fake_run(NO_RECOVERY_KEY)):
        check = mod.check(_make_profile())
        fix = mod.fix(check, Mode.MANUAL)
...
def test_emitted_codes_are_declared():
    ...
    assert all(c.startswith("security.win_bitlocker_check.") for c in declared)
